...
| **Automated population** | `envtool pull` resolves values from configured sources and writes the `.env` file |
...
```
envtool pull --schema config/frontend.env-schema.yaml --env staging
```
...
```
envtool check --schema config/frontend.env-schema.yaml
```
...
```
envtool list --schema config/backend.env-schema.yaml
```
...
Running `envtool pull -e staging` resolves:
...
envtool pull -s config/frontend.env-schema.yaml --dry-run

# Generate local frontend .env
envtool pull -s config/frontend.env-schema.yaml
...
envtool list -s config/backend.env-schema.yaml -e production --format json

# Non-interactive pull (CI/CD — skip manual prompts)
envtool pull -s config/frontend.env-schema.yaml -e staging --non-interactive

# Override output path
envtool pull -s config/backend.env-schema.yaml -o /tmp/test.env --force
```
