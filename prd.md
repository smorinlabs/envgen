...
```
envgen pull -c env.dev.yaml -e staging
```
...
#   envgen pull -c env.dev.yaml -e staging --interactive --force
...
```
Schema: env.dev.yaml (Project secrets)
...
```
$ envgen pull -c env.dev.yaml -e staging --dry-run
...
```
$ envgen pull -c env.dev.yaml -e staging --force
...
envgen pull -c env.dev.yaml --dry-run

# Generate local frontend .env
envgen pull -c env.dev.yaml --interactive
...
envgen list -c env.dev.yaml -e production --format json

# Interactive pull (prompt for manual values)
envgen pull -c env.dev.yaml -e staging --interactive

# Override output path
envgen pull -c env.dev.yaml --destination /tmp/test.env --force
```
...
| **Wrapper scripts** | The Makefile provides repository-quality checks via `make check`; schema operations are run directly with `envgen <command> -c <schema-file> ...`. |
