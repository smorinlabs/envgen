...
> **Partially superseded.** The hook manager chosen below is superseded by
> [ADR-0017](0017-adopt-lefthook-as-git-hook-manager.md), which replaced
> `pre-commit` with lefthook and deleted `.pre-commit-config.yaml`. Read
> "pre-commit stage" and "pre-push stage" below as the lefthook stages of the
> same names. Everything else in this record remains in force — the Makefile is
> still the single source of truth for quality gates, and CI, release, and local
> hooks still call those targets.
